...
GCutG* pin1pin2_gate=0;
GCutG* pin1sssd_gate=0;
...
void HandleUML(TRuntimeObjects& obj) {
 ...
 // }
  ...
  //if(e1_e2 && e1_e2->IsInside(uml->GetPin1().GetEnergy(),uml->GetPin2().GetEnergy())) {
    ...
    if(pin1pin2_gate && pin1sssd_gate &&  pin1pin2_gate -> IsInside(uml->GetPin1().GetEnergy(),uml->GetPin2().GetEnergy())
        && pin1sssd_gate -> IsInside(uml->GetPin1().GetEnergy(),uml->GetSssdEnergy()) ){
...
void SetGates(TRuntimeObjects &obj) {
  ...
  while(TObject *key = iter.Next()) {
    ...
    if(strcmp(gate->GetTag(),"pid")==0) {
      //std::cout << gate->GetName() << "\t" << gate->GetTag() << std::endl;
      pid_gates.push_back(gate);
    }
    if(strcmp(gate->GetTag(),"pidz")==0) {
      //std::cout << gate->GetName() << "\t" << gate->GetTag() << std::endl;
      zpid_gates.push_back(gate);
    }
    if(strcmp(gate->GetName(),"pin1_pin2_noreaction")==0) {
      pin1pin2_gate = gate;
    }
    if(strcmp(gate->GetName(),"pin1_sssd_noreactions")==0) {
      pin1sssd_gate = gate;
    }
    if(strcmp(gate->GetName(),"pin1_imp")==0) {
      pin1imp_gate = gate;
    }
    if(strcmp(gate->GetName(),"tof_cut")==0) {
      tof_cut = gate;
    }
  }
  gates_set=true;
...
//  std::cout<<"gre size: "<<gretina_map.size()<<std::endl;
  ...
  while ( gretina_map.size()> 0 && uml_map.size()>0 && (itr->first - uml_map.begin()->first > twin)) {
    ...
    bool pidflag = false;
    if(pin1pin2_gate == NULL || pin1sssd_gate == NULL) pidflag = true;
    else if(pin1pin2_gate -> IsInside(uml.GetPin1().GetEnergy(),uml.GetPin2().GetEnergy())
       && pin1sssd_gate -> IsInside(uml.GetPin1().GetEnergy(),uml.GetSssdEnergy()) ) pidflag = true;
...
//                  std::cout<<"fts: l+1  "<<tmp_hit.Timestamp()- fts<<std::endl;
                ...
                obj.FillHistogramSym(dirname,Form("gg_m2_%s",gate->GetName()),2000,0,4000,en1,2000,0,4000,en2);
             }
          }
          size_t l = 0; 
          while(l<store_ghit.size()){
             size_t k;
             float coll_e = store_ghit.at(l)->second.GetCoreEnergy();
             for(k = l+1;k<store_ghit.size(); k++){
               if(store_ghit.at(k)->first - store_ghit.at(l)->first > 100) break;
               coll_e += store_ghit.at(k)->second.GetCoreEnergy();
             }
             obj.FillHistogram(dirname,Form("gamma_time_uml_coll_%s",gate->GetName()),1000,-twin/100.,twin/100.,(store_ghit.at(l)->first - fts) / 100., 4000, 0, 8000, coll_e);
             obj.FillHistogram(dirname,Form("gamma_time_uml_coll_narrow_%s",gate->GetName()),1000,-300,300.,(store_ghit.at(l)->first - fts) / 100., 4000, 0, 8000, coll_e);
             l = k;
          }
        }
      }
...
// }
  ...
  if(uml && gretina) {
    ...
    if(tof_cut->IsInside(uml->GetTof(),uml->GetPin2().GetEnergy()))
    obj.FillHistogram("gretina_ddas_gated",3600,0,3600,(gretina->Timestamp())/1e8,
        500,-500,500,gretina->Timestamp()-  uml->Timestamp());

    obj.FillHistogram("gretina_ddaspin1",17000,0,17000,uml->GetPin1().GetEnergy(),
        500,-500,500,gretina->Timestamp()-  uml->Timestamp());
    obj.FillHistogram("gretina_ddaspin1_tdiff",3600,0,3600,gretina->Timestamp()/1e8,
        500,-500,500,gretina->Timestamp()-  uml->GetPin1().Timestamp());
    obj.FillHistogram("gretina_ddaspin2",17000,0,17000,uml->GetPin2().GetEnergy(),
        500,-500,500,gretina->Timestamp()-  uml->Timestamp());
    obj.FillHistogram("gretina_ddaspin2_tdiff",3600,0,3600,gretina->Timestamp(),
        500,-500,500,gretina->Timestamp()-  uml->GetPin2().Timestamp());
