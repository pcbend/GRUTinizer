...
void MakeJanus(TRuntimeObjects& obj, TJanusDDAS& janus, GCutG* janus_gate) {
  ...
  for(auto& hit : janus.GetSingleHits(0)) {
    if(hit.GetDetnum() == 1) {
      obj.FillHistogram(dirname,"Charge_Summary_Paired_Events",
                        26,0,26,hit.GetRing(),
                        3500,4000,35000,hit.GetBackHit().Charge());
      if(janus_gate->IsInside(hit.GetRing(),hit.GetBackHit().Charge())) {
        obj.FillHistogram(dirname,"Charge_Summary_Paired_Events_Gated",
                          26,0,26,hit.GetRing(),
                          3500,4000,35000,hit.GetBackHit().Charge());
      }
    }
