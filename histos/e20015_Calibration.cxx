...
void MakeSega(TRuntimeObjects& obj, TSega& sega) {
  ...
  for(unsigned int i=0;i<sega.Size();i++) {
    ...
    */
    ...
    //obj.FillHistogram(dirname,Form("CoreEn_v_time%02d",hit.GetDetnum()),2000,0,100000000000000,hit.Timestamp(),
    //                  4000,0,4000,hit.GetEnergy());
    ...
    if(num_segs >= 1) {
      //obj.FillHistogram(dirname,Form("CoreEn_with_Segs_det%02d",hit.GetDetnum()),
      //                  2000,0,4000,hit.GetEnergy());
      obj.FillHistogram(dirname,"CoreEn_with_Segs",18,0,18,hit.GetDetnum(),
                        2000,0,4000,hit.GetEnergy());
    }
    else {
      //obj.FillHistogram(dirname,Form("CoreEn_without_Segs_det%02d",hit.GetDetnum()),
      //                  2000,0,4000,hit.GetEnergy());
      obj.FillHistogram(dirname,"CoreEn_without_Segs",18,0,18,hit.GetDetnum(),
                        2000,0,4000,hit.GetEnergy());
    }
    ...
    for(int j=0;j<num_segs;j++) {
      ...
      obj.FillHistogram(dirname,Form("Core_Seg_tdiff_det%02d",hit.GetDetnum()),
                        2000,-20000,20000,seg_ts-cc_ts);
...
void MakeGatedSeGAJanus(TRuntimeObjects& obj, TSega& sega, TJanusDDAS& janus, GCutG* janus_gate,
  ...
  static TSRIM srimB("32Si_in_196Pt");
