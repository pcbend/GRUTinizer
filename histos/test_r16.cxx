...
*/  
  ...
  for(auto& sect : janus.GetSectors()) {
	...
	obj.FillHistogram(dirname,Form("Sector_Ring_TDiff_UsableChans_det%d",sect.GetDetnum()),
        ...
        if(ring.GetSector() == 16 && ring.GetDetnum() == 0) {
	  obj.FillHistogram(dirname,"R16_SectorCharge_v_TDiff_UsableChans",
	   	            2500,-11000,11000,tdiff,
	 	            2000,0,30000,sect.Charge());
	
	  obj.FillHistogram(dirname,"R16_RingCharge_v_TDiff_UsableChans",
	   	            2500,-11000,11000,tdiff,
	 	            2000,0,30000,ring.Charge());
	
          obj.FillHistogram(dirname,"R16_SectorCharge_v_RingCharge_UsableChans",
		            2000,0,30000,ring.Charge(),
		            2000,0,30000,sect.Charge());

	  obj.FillHistogram(dirname,"R16_Sector_Ring_TDiff_UsableChans",
			    6000,-11000,11000,tdiff);
        }
...
void MakeGatedSeGAJanus(TRuntimeObjects& obj, TSega& sega, TJanusDDAS& janus, GCutG* janus_gate,
  ...
  for(auto& j_hit : janus.GetHits(lvl)) {
	...
	if(!time_gate->IsInside(tdiff,s_energy)) {
      ...
      obj.FillHistogram(dirname,"DopplerEnergy_v_Ring",26,0,26,j_hit.GetRing(),4000,0,4000,dop_en);
/*
...
*/
	...
	obj.FillHistogram(dirname,"jZ_Scan",100,-0.5,0.51,tmp_zoff,
      ...
      obj.FillHistogram(dirname,"ReconEnergy_v_Ring",26,0,26,j_hit.GetRing(),4000,0,4000,recon_en);
