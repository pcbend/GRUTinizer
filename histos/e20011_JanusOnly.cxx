...
void BasicJanusSpectra(TRuntimeObjects& obj, TJanusDDASHit& hit, std::string dirname, std::string h_suf) {
  ...
  obj.FillHistogram(dirname,Form("%sSectorCharge_v_Ring_det%d",h_suf.c_str(),hit.GetDetnum()),
		    26,0,26,hit.GetRing(),10000,0,100000,hit.Charge());

  obj.FillHistogram(dirname,Form("%sSectorCharge_v_Sector_det%d",h_suf.c_str(),hit.GetDetnum()),
		    34,0,34,hit.GetSector(),10000,0,100000,hit.Charge());

  obj.FillHistogram(dirname,Form("%sRingCharge_v_Ring_det%d",h_suf.c_str(),hit.GetDetnum()),
		    26,0,26,hit.GetRing(),10000,0,100000,hit.GetBackHit().Charge());

  obj.FillHistogram(dirname,Form("%sSectorEnergy_v_Sector_det%d",h_suf.c_str(),hit.GetDetnum()),
		    34,0,34,hit.GetSector(),10000,0,100000,hit.GetEnergy());

  obj.FillHistogram(dirname,Form("%sRingEnergy_v_Ring_det%d",h_suf.c_str(),hit.GetDetnum()),
		    26,0,26,hit.GetRing(),10000,0,100000,hit.GetBackHit().GetEnergy());
