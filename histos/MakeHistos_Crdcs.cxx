...
void MonitorCrdcs(TRuntimeObjects& obj){
  ...
  obj.FillHistogram(dirname,"Crdc1_xy",800,-0,0,crdc1_x,800,-0,0,crdc1_y);
  obj.FillHistogram(dirname,"Crdc2_xy",800,-0,0,crdc2_x,800,-0,0,crdc2_y);
  ...
  if(s800 && s800->GetTrigger().GetRegistr() <100 && s800->GetTrigger().GetRegistr()>0){
    ...
    histname = Form("crdc1_xy_%d",s800->GetTrigger().GetRegistr());
    obj.FillHistogram(dirname,histname.Data(),800,-400,400,crdc1_x,800,-400,400,crdc1_y);
    histname = Form("crdc2_xy_%d",s800->GetTrigger().GetRegistr());
    obj.FillHistogram(dirname,histname.Data(),800,-400,400,crdc2_x,800,-400,400,crdc2_y);
  }
...
void IncomingPID(TRuntimeObjects &obj){
  ...
  obj.FillHistogram(dirname,"TOF-dE-Sci-raw",5000,-10000,0,s800->GetRawOBJ_MESY(0)-s800->GetRawXF_MESY(0),1000,0,40000,obj_sci->GetOBJHit(0).GetEnergy());
  histname = Form("TOF-dE-Sci-raw_%d",s800->GetTrigger().GetRegistr());
  obj.FillHistogram(dirname,histname.Data(),5000,-10000,0,s800->GetRawOBJ_MESY(0)-s800->GetRawXF_MESY(0),1000,0,40000,obj_sci->GetOBJHit(0).GetEnergy());
...
void MonitorTOF(TRuntimeObjects &obj){
  ...
  obj.FillHistogram(dirname,"OBJ_TAC",4000,-2000,2000,s800->GetOBJRaw_TAC());
  obj.FillHistogram(dirname,"XFP_TAC",4000,-2000,2000,s800->GetXFRaw_TAC());
  obj.FillHistogram(dirname,"OBJ-XFP",4000,-2000,2000,s800->GetXFRaw_TAC()-s800->GetOBJRaw_TAC());
  ...
  obj.FillHistogram(dirname,"XFP_multi_1d",100,0,100,mtof.XfpSize());
  obj.FillHistogram(dirname,"OBJ_multi_1d",100,0,100,mtof.ObjSize());
  obj.FillHistogram(dirname,"E1Up_multi_1d",100,0,100,mtof.E1UpSize());
  obj.FillHistogram(dirname,"XFP_OBJ_multi_2d",50,0,50,mtof.XfpSize(),50,0,50,mtof.ObjSize());
  obj.FillHistogram(dirname,"XFP_E1Up_multi_2d",50,0,50,mtof.XfpSize(),50,0,50,mtof.E1UpSize());
  for(int x = 0; x < mtof.XfpSize(); x++){
    obj.FillHistogram(dirname,"XFP_Mesy_1d",1000,0,100000,s800->GetRawXF_MESY(x));
  }

  for(int x = 0; x < mtof.ObjSize(); x++){
    obj.FillHistogram(dirname,"OBJ_Mesy_1d",1000,0,100000,s800->GetRawOBJ_MESY(x));
  }
  for(int x = 0; x < mtof.E1UpSize(); x++){
    obj.FillHistogram(dirname,"E1_Mesy_1d",1000,-0,100000,s800->GetRawE1_MESY(x));
  }
  ...
  if(s800 && s800->GetTrigger().GetRegistr() <100 && s800->GetTrigger().GetRegistr()>0){
     TString histname = Form("XFP_%d",s800->GetTrigger().GetRegistr());
     for(int x = 0; x < mtof.XfpSize(); x++)
       obj.FillHistogram(dirname,histname.Data(),1000,-10000,40000,s800->GetRawXF_MESY(x));
     histname = Form("OBJ_%d",s800->GetTrigger().GetRegistr());
     for(int x = 0; x < mtof.ObjSize(); x++)
       obj.FillHistogram(dirname,histname.Data(),1000,-10000,40000,s800->GetRawOBJ_MESY(x));
     histname = Form("E1_%d",s800->GetTrigger().GetRegistr());
     for(int x = 0; x < mtof.E1UpSize(); x++)
       obj.FillHistogram(dirname,histname.Data(),1000,-10000,40000,s800->GetRawE1_MESY(x));
     histname = Form("XFP-OBJ_%d",s800->GetTrigger().GetRegistr());
     for(int x = 0; x < mtof.ObjSize(); x++)
     for(int y = 0; y < mtof.XfpSize();y++){
       obj.FillHistogram(dirname,histname.Data(),5000,-8000,-3000,s800->GetRawOBJ_MESY(x)-s800->GetRawXF_MESY(y));
     }
     histname = Form("XFP-OBJ-TAC_%d",s800->GetTrigger().GetRegistr());
     obj.FillHistogram(dirname,histname.Data(),40000,-2000,2000,s800->GetXFRaw_TAC()-s800->GetOBJRaw_TAC());
  }
}
...
void DrawTrigReg(TRuntimeObjects &obj){
  TS800 *s800 = obj.GetDetector<TS800>();
  if(s800)
  obj.FillHistogram("triggers",20,0,20,s800->GetTrigger().GetRegistr());
}

void MonitorGretina(TRuntimeObjects &obj){
  TGretina *gretina = obj.GetDetector<TGretina>();
  if(!gretina) return;

  std::string dirname = "Gretina_Raw";

  for(size_t x = 0 ; x < gretina->Size(); x++){
    auto ghit = gretina->GetGretinaHit(x);
    int quad = HoleQMap[ghit.GetHoleNumber()];
    int pos = (ghit.GetCrystalId()%4) + 1;
    obj.FillHistogram(dirname,"xtal_pad",130,0,130,ghit.GetPad(),44,0,44,quad*4+pos-5);
    if(!ghit.GetPad()){
      obj.FillHistogram(dirname,"theta_phi",180,0,TMath::Pi(),ghit.GetTheta(),180,0,2*TMath::Pi(),ghit.GetPhi());
      obj.FillHistogram(dirname,"xtal_energy",4000,0,4000,ghit.GetCoreEnergy(),44,0,44,quad*4+pos-5);
    }
    
  }
}

void GammaRayTag(TRuntimeObjects &obj){
  if(pidcuts.size()<=0) return;

  auto s800 = obj.GetDetector<TS800>();
  auto gretina = obj.GetDetector<TGretina>();

  if(!s800 || !gretina) return;
  ...
  std::string dirname = "GammaRayTag";

  for(size_t x = 0 ; x<pidcuts.size(); x++){
    auto gate = pidcuts.at(x);
    if(gate->IsInside(tof,dE)){
      for(size_t y = 0; y < gretina->Size(); y++){
         double tdiff = s800->Timestamp()-gretina->GetGretinaHit(y).Timestamp();
         if(tdiff<50 || tdiff>150) continue;
         obj.FillHistogram(dirname,gate->GetName(),4000,0,4000,gretina->GetGretinaHit(y).GetDoppler(0.38));
         obj.FillHistogram(dirname,Form("energy_vs_theta_%s",gate->GetName()),180,0,TMath::Pi(),gretina->GetGretinaHit(y).GetTheta(),4000,0,4000,gretina->GetGretinaHit(y).GetDoppler(0.38));
      }
    }
  }
}
