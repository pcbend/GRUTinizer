    ...
    for(unsigned int x=0;x<labr->Size();x++) {
      if(labr->GetLaBrHit(x).GetChannel()==15) {
        ...
        for(unsigned int z=0;z<labr->Size();z++) {
          ...
          histname=Form("chan_15_time_chan_%i_time",labr->GetLaBrHit(z).GetChannel());
          obj.FillHistogram(dirname,histname,3000,10000,13000,labr->GetRefTime() - labr->GetLaBrHit(z).GetTime(),
              3000,10000,13000,labr->GetRefTime() - labr->GetLaBrHit(x).GetTime());
        }
      ...
      //                                   32,0,32,labr->GetLaBrHit(x).GetChannel());
      if(s800) {
        histname=Form("summary_time_reg_%i",s800->GetReg());
        obj.FillHistogram(dirname,histname,4000,0,64000,labr->GetLaBrHit(x).GetTime(),
            32,0,32,labr->GetLaBrHit(x).GetChannel());
        //if(labr->GetRefTime() >100 && labr->GetLaBrHit(x).GetTime() >100 ) {

        if(labr->GetRefTime() >100 && labr->GetLaBrHit(x).GetTime() >100 &&  labr->GetLaBrHit(x).Charge() >100 && s800->GetReg()==1) {
          //histname=Form("summary_timeref_reg1_%i",s800->GetReg());
          //obj.FillHistogram(dirname,histname,3000,11000,14000,labr->GetRefTime() - labr->GetLaBrHit(x).GetTime(),
          //                                   32,0,32,labr->GetLaBrHit(x).GetChannel());


          histname = Form("chan_%i_timeref_charge_reg1",labr->GetLaBrHit(x).GetChannel());
        ...
        if(labr->GetRefTime() >100 && labr->GetLaBrHit(x).GetTime() >100 &&  labr->GetLaBrHit(x).Charge() >100 ) {
          histname=Form("summary_timeref_reg_%i",s800->GetReg());
          obj.FillHistogram(dirname,histname,3000,10000,13000,labr->GetRefTime() - labr->GetLaBrHit(x).GetTime(),
              32,0,32,labr->GetLaBrHit(x).GetChannel());

          histname = Form("chan_%i_timeref_charge",labr->GetLaBrHit(x).GetChannel());
          obj.FillHistogram(dirname,histname,3000,10000,13000,labr->GetRefTime() - labr->GetLaBrHit(x).GetTime(),
              1000,0,4000,labr->GetLaBrHit(x).Charge());
