...
static long prev_ring_ts_ds [24] = {};
static long prev_ring_ts_us [24] = {};
void MakeJanus(TRuntimeObjects& obj, TJanusDDAS& janus) {
  ...
  for(auto& ring : rings) {
    if(ring.GetDetnum() == 1) { 
      ...
      if(prev_ring_ts_ds[ringNum-1] > 0) {
        obj.FillHistogram(dirname,Form("Ring%d_Timestamp_det%d",ringNum,ring.GetDetnum()),10000000,0,1000000000,ring.Timestamp()-prev_ring_ts_ds[ringNum-1]);
      }
      prev_ring_ts_ds[ringNum-1] = ring.Timestamp();
    }
    if(ring.GetDetnum() == 0) { 
      ringNum = ring.GetSector();
      if(prev_ring_ts_us[ringNum-1] > 0) {
        obj.FillHistogram(dirname,Form("Ring%d_Timestamp_det%d",ringNum,ring.GetDetnum()),10000000,0,1000000000,ring.Timestamp()-prev_ring_ts_us[ringNum-1]);
      }
      prev_ring_ts_us[ringNum-1] = ring.Timestamp();
    }
  }
