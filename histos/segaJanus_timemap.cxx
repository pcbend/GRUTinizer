...
void MakeGatedJanus(TRuntimeObjects& obj, TJanusDDAS& janus, GCutG* gate, GCutG* janus_time_gate) {
  ...
  for(auto& hit : janus.GetHits(lvl)) {
    ...
    if(!janus_time_gate->IsInside(hit.GetBackHit().Timestamp() - hit.Timestamp(),hit.Charge())) {
      continue;
    }
    if(!janus_time_gate->IsInside(hit.GetBackHit().Timestamp() - hit.Timestamp(),hit.GetBackHit().Charge())) {
      continue;
    }
...
void MakeSeGAJanus(TRuntimeObjects& obj, TSega& sega, TJanusDDAS& janus, GCutG* janus_time_gate) {
  ...
  for(auto& j_hit : janus.GetHits(lvl)) {
    ...
    if(!janus_time_gate->IsInside(j_hit.GetBackHit().Timestamp() - j_hit.Timestamp(),j_hit.Charge())) {
      continue;
    }
    if(!janus_time_gate->IsInside(j_hit.GetBackHit().Timestamp() - j_hit.Timestamp(),j_hit.GetBackHit().Charge())) {
      continue;
    }
...
void MakeGatedSeGAJanus(TRuntimeObjects& obj, TSega& sega, TJanusDDAS& janus, GCutG* janus_gate,
			GCutG* time_gate, GCutG* janus_time_gate) {
...
void MakeHistograms(TRuntimeObjects& obj) {
  ...
  if(gates_loaded!=gates->GetSize()) {
	...
	gates_loaded++;
      ...
      else if(!tag.compare("janus_time_energy")) {
        janus_time_gate = gate;
	gates_loaded++;
        std::cout << "Janus Time Gate: " << name << std::endl;
      }
...
*/
  ...
  if(janus) {
    ...
    for(auto &gate : janus_gates) {
      MakeGatedJanus(obj,*janus,gate,janus_time_gate);
    }
  }

  if(sega && janus) {

    MakeSeGAJanus(obj,*sega,*janus,janus_time_gate);
    for(auto &gate : janus_gates) {
      MakeGatedSeGAJanus(obj,*sega,*janus,gate,time_gate,janus_time_gate);
    }
  }
