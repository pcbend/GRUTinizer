...
void MakeSega(TRuntimeObjects& obj, TSega& sega) {
  ...
  for(unsigned int i=0;i<sega.Size();i++) {
    ...
    obj.FillHistogram("SeGA_Detectors",Form("SegSummary_det%02d",hit.GetDetnum()),
		      34,0,34,hit.GetMainSegnum(),4000,0,4000,hit.GetEnergy());
    */

    for(int j=0;j<num_segs;j++) {
      TSegaSegmentHit seg = hit.GetSegment(j);
      obj.FillHistogram(dirname,Form("SegChargeSummary_det%02d",hit.GetDetnum()),34,0,34,seg.GetSegnum(),
                        3000,0,36000,seg.Charge());
    }

    for(int j=0;j<num_segs;j++) {
      TSegaSegmentHit seg = hit.GetSegment(j);
      obj.FillHistogram(dirname,Form("SegEnergySummary_det%02d",hit.GetDetnum()),34,0,34,seg.GetSegnum(),
                        4000,0,4000,seg.GetEnergy());
    }
    ...
    if(hit.GetDetnum() == 1) {
      for(unsigned int j=0;j<sega.Size();j++) {
        TSegaHit hit2 = sega.GetSegaHit(j);
        if((hit.GetEnergy() >= 1165 && hit.GetEnergy() <= 1181) && (hit2.GetDetnum() == 2)) {
          obj.FillHistogram(dirname,"Core_Time_Difference_det1-2_only",50,-1000,1000,hit.Timestamp()-hit2.Timestamp(),375,0,1500,hit2.GetEnergy());
        }
      }
    }
    if(hit.GetDetnum() == 1) {
      for(unsigned int j=0;j<sega.Size();j++) {
        TSegaHit hit2 = sega.GetSegaHit(j);
        if((hit.GetEnergy() >= 1165 && hit.GetEnergy() <= 1181) && (hit2.GetDetnum() == 3)) {
          obj.FillHistogram(dirname,"Core_Time_Difference_det1-3_only",50,-1000,1000,hit.Timestamp()-hit2.Timestamp(),375,0,1500,hit2.GetEnergy());
        }
      }
    }
    if(hit.GetDetnum() == 2) {
      for(unsigned int j=0;j<sega.Size();j++) {
        TSegaHit hit2 = sega.GetSegaHit(j);
        if((hit.GetEnergy() >= 1165 && hit.GetEnergy() <= 1181) && (hit2.GetDetnum() == 1)) {
          obj.FillHistogram(dirname,"Core_Time_Difference_det2-1_only",50,-1000,1000,hit.Timestamp()-hit2.Timestamp(),375,0,1500,hit2.GetEnergy());
        }
      }
    }
    if(hit.GetDetnum() == 2) {
      for(unsigned int j=0;j<sega.Size();j++) {
        TSegaHit hit2 = sega.GetSegaHit(j);
        if((hit.GetEnergy() >= 1165 && hit.GetEnergy() <= 1181) && (hit2.GetDetnum() == 3)) {
          obj.FillHistogram(dirname,"Core_Time_Difference_det2-3_only",50,-1000,1000,hit.Timestamp()-hit2.Timestamp(),375,0,1500,hit2.GetEnergy());
        }
      }
    }
    if(hit.GetDetnum() == 3) {
      for(unsigned int j=0;j<sega.Size();j++) {
        TSegaHit hit2 = sega.GetSegaHit(j);
        if((hit.GetEnergy() >= 1165 && hit.GetEnergy() <= 1181) && (hit2.GetDetnum() == 1)) {
          obj.FillHistogram(dirname,"Core_Time_Difference_det3-1_only",50,-1000,1000,hit.Timestamp()-hit2.Timestamp(),375,0,1500,hit2.GetEnergy());
        }
      }
    }
    if(hit.GetDetnum() == 3) {
      for(unsigned int j=0;j<sega.Size();j++) {
        TSegaHit hit2 = sega.GetSegaHit(j);
        if((hit.GetEnergy() >= 1165 && hit.GetEnergy() <= 1181) && (hit2.GetDetnum() == 2)) {
          obj.FillHistogram(dirname,"Core_Time_Difference_det3-2_only",50,-1000,1000,hit.Timestamp()-hit2.Timestamp(),375,0,1500,hit2.GetEnergy());
        }
      }
    }
/*
    if(hit.GetDetnum() == 2) {
      for(unsigned int j=0;j<sega.Size();j++) {
        TSegaHit hit2 = sega.GetSegaHit(j);
        if((hit.GetEnergy() >= 1165 && hit.GetEnergy() <= 1181) && (hit2.GetDetnum() == 1)) {
          obj.FillHistogram(dirname,"Core_Time_Difference",200,-1000,1000,hit2.Timestamp()-hit.Timestamp(),1000,0,4000,hit2.GetEnergy());
        }
      }
    }
*/
...
void MakeGatedSeGAJanus(TRuntimeObjects& obj, TSega& sega, TJanusDDAS& janus, GCutG* janus_gate,
  ...
  static TSRIM srimB("116Sn_in_196Pt");
