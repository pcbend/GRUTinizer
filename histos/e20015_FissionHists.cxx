...
void MakeJanus(TRuntimeObjects& obj, TJanusDDAS& janus) {
  ...
  obj.FillHistogram(dirname,"Mult",20,0,20,janus.GetAllChannels().size());
...
void MakeGatedJanus(TRuntimeObjects& obj, TJanusDDAS& janus, GCutG* pid_gate) {

  std::string dirname = "GatedJanus";
  int lvl = 1;

  std::string tag = pid_gate->GetTag();
  ...
  for(auto& hit : janus.GetHits(lvl)) {
    ...
    TVector3 pos = hit.GetPosition(false);

    obj.FillHistogram(dirname,Form("Y_v_X_det%d",hit.GetDetnum()),
  		      200,-4,4,pos.X(),
  		      200,-4,4,pos.Y());
  }
...
void MakeGatedSeGAJanus(TRuntimeObjects& obj, TSega& sega, TJanusDDAS& janus, GCutG* time_gate, GCutG* pid_gate) {

  std::string dirname = Form("SeGAJanus_%s",time_gate->GetName());
  int lvl = 1;
  ...
  for(auto& j_hit : janus.GetHits(lvl)) {
    ...
    */
	...
	continue;
      ...
      if(j_hit.GetDetnum() != detnum || !pid_gate->IsInside(j_hit.GetSector(),j_hit.Charge())) {
        continue;
      }
      ...
      if(janus.GetHits(1).size() == 2) {
        ...
        obj.FillHistogram(dirname,"Angle_Mult2",100,-10,190,angle);
      }

      auto s_pos = s_hit.GetPosition(false);
      auto j_pos = j_hit.GetPosition(false);
      j_pos.SetPhi(j_pos.Phi() + TMath::Pi());
      auto particleGammaAngle = s_pos.Angle(j_pos)*TMath::RadToDeg();

      obj.FillHistogram(dirname,"Core_Energy",5000,0,5000,s_hit.GetEnergy());

      obj.FillHistogram(dirname,"ParticleGammaAngle_v_CoreEnergy",
                        2000,0,4000,s_hit.GetEnergy(),
                        90,0,180,particleGammaAngle);
	...
	zoff = GValue::Value("Sega_Z_offset");
      ...
      obj.FillHistogram(dirname,Form("ParticleCharge_v_DopplerEnergy_025_jdet%d",j_hit.GetDetnum()),
			2000,0,5000,dop_en,
			1000,0,30000,j_hit.Charge());
      
      obj.FillHistogram(dirname,"Doppler_Energy_025",4000,0,4000,dop_en);

      beta = 0.030;
      dop_en = s_hit.GetDoppler(beta,j_pos,TVector3(0,0,zoff));
      obj.FillHistogram(dirname,"Doppler_Energy_030",4000,0,4000,dop_en);

      beta = 0.035;
      dop_en = s_hit.GetDoppler(beta,j_pos,TVector3(0,0,zoff));
      obj.FillHistogram(dirname,"Doppler_Energy_035",4000,0,4000,dop_en);

      beta = 0.040;
      dop_en = s_hit.GetDoppler(beta,j_pos,TVector3(0,0,zoff));
      obj.FillHistogram(dirname,"Doppler_Energy_040",4000,0,4000,dop_en);

      beta = 0.045;
      dop_en = s_hit.GetDoppler(beta,j_pos,TVector3(0,0,zoff));
      obj.FillHistogram(dirname,"Doppler_Energy_045",4000,0,4000,dop_en);

      beta = 0.050;
      dop_en = s_hit.GetDoppler(beta,j_pos,TVector3(0,0,zoff));
      obj.FillHistogram(dirname,"Doppler_Energy_050",4000,0,4000,dop_en);

      beta = 0.055;
      dop_en = s_hit.GetDoppler(beta,j_pos,TVector3(0,0,zoff));
      obj.FillHistogram(dirname,"Doppler_Energy_055",4000,0,4000,dop_en);

      beta = 0.060;
      dop_en = s_hit.GetDoppler(beta,j_pos,TVector3(0,0,zoff));
      obj.FillHistogram(dirname,"Doppler_Energy_060",4000,0,4000,dop_en);
...
void MakeHistograms(TRuntimeObjects& obj) {
  ...
  if(gates_loaded!=gates->GetSize()) {
	...
	gates_loaded++;
        ...
        gates_loaded++;
        std::cout << "PID Gate: << " << gate->GetName() << std::endl;
      }
      else {
  ...
  if(sega && janus) {
    ...
    for(auto &gate : time_gates) {
      for(auto &pid_gate : pid_gates) {
        MakeGatedSeGAJanus(obj,*sega,*janus,gate,pid_gate);
      }
    }    
  }
