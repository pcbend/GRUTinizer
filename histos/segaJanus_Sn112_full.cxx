...
void BasicJanusSpectra(TRuntimeObjects& obj, TJanusHit& hit, std::string dirname, std::string h_suf) {

  const TVector3 pos = hit.GetPosition(true);
...
void MakeJanus(TRuntimeObjects& obj, TJanus& janus) {
  ...
  std::vector<TJanusHit> rings;
  std::vector<TJanusHit> sectors;
  ...
  for(size_t i=0; i<janus.GetAllChannels().size(); i++) {
    auto &chan = janus.GetAllChannels().at(i);
    ...
    for(size_t j=i+1;j<janus.GetAllChannels().size();j++) {
      auto &chan1 = janus.GetAllChannels().at(j);
...
void MakeGatedJanus(TRuntimeObjects& obj, TJanus& janus, GCutG* gate) {
...
void MakeSeGAJanus(TRuntimeObjects& obj, TSega& sega, TJanus& janus) {
...
void MakeGatedSeGAJanus(TRuntimeObjects& obj, TSega& sega, TJanus& janus, GCutG* janus_gate,
			GCutG* time_gate) {
  ...
  bool recoil = false;
  if((!name.compare("pt196_ds")) || (!name.compare("pb208"))) {
    recoil = true;
  }
  ...
  for(auto& j_hit : janus.GetAllHits()) {
    ...
    TVector3 j_pos = j_hit.GetPosition(true);
    double beta;
    double recon_beta;
	...
	if(!time_gate->IsInside(tdiff,s_energy)) {
      ...
      if(detNum == 1) {
        if(recoil == false) {
          div_t divresult = div(j_hit.GetRing()-1,6);
          int exptNum = divresult.quot + 1;
	  obj.FillHistogram(dirname,Form("Doppler_Energy_Expt%d",exptNum),4000,0,4000,dop_en);
	}
	else {
          div_t divresult = div(j_hit.GetRing()-1,12);
	  int exptNum = divresult.quot + 1;
	  obj.FillHistogram(dirname,Form("Doppler_Energy_Expt%d",exptNum),4000,0,4000,dop_en);
	}
      }
	...
	planeAng_rec += TMath::TwoPi();
      ...
      if(detNum == 1) {
        if(recoil == false) {
          div_t divresult = div(j_hit.GetRing()-1,6);
          int exptNum = divresult.quot + 1;
	  obj.FillHistogram(dirname,Form("Recon_Energy_Expt%d",exptNum),4000,0,4000,recon_en);
          obj.FillHistogram(dirname,Form("Recon_Energy_Summary%d",exptNum),18,0,18,s_hit.GetDetnum(),4000,0,4000,recon_en);
          obj.FillHistogram(dirname,Form("ReconTheta_Correlation%d",exptNum),4000,0,4000,s_energy,180,0,180,recon_angle);
          obj.FillHistogram(dirname,Form("ReconPhi_Correlation%d",exptNum),4000,0,4000,s_energy,32,0,thing,planeAng_rec);
	}
	else {
          div_t divresult = div(j_hit.GetRing()-1,12);
	  int exptNum = divresult.quot + 1;
      ...
      for(int i=1;i<61;i++) {
        double tmp_zoff = -1.0 + 0.05*i;
        obj.FillHistogram(dirname,Form("Recon_sZ_Scan_Det%d",s_hit.GetDetnum()),60,-1.0,2.05,tmp_zoff,
                          2000,0,4000,s_hit.GetDoppler(recon_beta,recon_pos,TVector3(0,0,tmp_zoff)));
        obj.FillHistogram(dirname,Form("Recon_sZ_Scan_Det%d_Slice%d",s_hit.GetDetnum(),s_hit.GetMapSlicenum()),
                          60,-1.0,2.05,tmp_zoff,2000,0,4000,s_hit.GetDoppler(recon_beta,recon_pos,TVector3(0,0,tmp_zoff)));
      }
