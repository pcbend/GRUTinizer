...
void MakeGatedSeGAJanus(TRuntimeObjects& obj, TSega& sega, TJanusDDAS& janus, GCutG* janus_gate, GCutG* time_gate) {
  ...
  for(auto& j_hit : janus.GetHits(lvl)) {
	...
	if(!time_gate->IsInside(tdiff,s_energy)) {
      ...
      if(dop_en >= 1240 && dop_en <= 1275) {
        obj.FillHistogram(dirname,Form("Doppler_SectorCharge_v_Ring_det%d",j_hit.GetDetnum()),
		          26,0,26,j_hit.GetRing(),3000,0,30000,j_hit.Charge());
        obj.FillHistogram(dirname,"Doppler_Energy_Gammagate",4000,0,4000,dop_en);
      }
	...
	obj.FillHistogram(dirname,"jZ_Scan",100,-0.5,0.51,tmp_zoff,
      ...
      if(recon_en >= 1240 && recon_en <= 1275) {
        obj.FillHistogram(dirname,Form("Recon_SectorCharge_v_Ring_det%d",j_hit.GetDetnum()),
		          26,0,26,j_hit.GetRing(),3000,0,30000,j_hit.Charge());
      }
