...
void MakeHistograms(TRuntimeObjects& obj) {
    ...
    TOBJ *objs = obj.GetDetector<TOBJ>();
...
//        long long tdiff = objs->Timestamp()-first_ts;
        ...
        total_events++;
        obj.FillHistogram("ddas_dir","total_events_vs_ts",360,0,360,ohit.Timestamp()/1e8,1000,0,1e7,total_events);
        if(tdiff>=6000000) std::cout<<"tdiff: "<<tdiff<<std::endl;
        first_ts=ohit.Timestamp();
...
//                                  4000,0,4000,hit.GetIntPreampEng(z));
                ...
                if(quad  == 1 && p == 2 && hit.GetSegmentId(z) == 29) {
                    obj.FillHistogram(dirname,Form("seg_sec_Q%02i_p%01i_ch%02i",quad,p,hit.GetSegmentId(z)),3000,0,3000,hit.GetTime()/1e8,4000,0,4000,hit.GetIntPreampEng(z));
                }
