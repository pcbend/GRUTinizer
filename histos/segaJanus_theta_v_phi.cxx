...
void MakeGatedJanus(TRuntimeObjects& obj, TJanusDDAS& janus, GCutG* gate) {
  ...
  for(auto& hit : janus.GetHits(lvl)) {
    ...
    TVector3 j_pos = hit.GetPosition(false);
    janusRing = hit.GetRing();

    offset = TVector3(GValue::Value("Janus_X_offset"),GValue::Value("Janus_Y_offset"),GValue::Value("Janus_Z_offset"));
    rnd = double(std::rand())/double(RAND_MAX);
    if(detNum == 1) {
      janusTheta = (rnd*(std::atan((1.1 + 0.1*(janusRing))/2.6)-std::atan((1.1+0.1*(janusRing-1))/2.6)) + std::atan((1.1 + 0.1*(janusRing-1))/2.6))*180/TMath::Pi();
    }
    else{
      janusTheta = 180 - (rnd*(std::atan((1.1 + 0.1*(janusRing))/3.6)-std::atan((1.1+0.1*(janusRing-1))/3.6)) + std::atan((1.1 + 0.1*(janusRing-1))/3.6))*180/TMath::Pi();
    }
    //std::cout << janusRing << "," << std::atan((1.1+0.1*(janusRing-1))/2.6)*180/TMath::Pi() << "," << std::atan((1.1+0.1*(janusRing))/2.6)*180/TMath::Pi() << std::endl;
    rnd = double(std::rand())/double(RAND_MAX);
    janusPhi = j_pos.Phi()*180/TMath::Pi()-5.625+rnd*11.25;
    //std::cout << j_pos.Phi()*180/TMath::Pi() << std::endl;
    if(janusPhi < -180) {
      janusPhi = janusPhi + 360;
    }
    if(detNum == 1) {
      obj.FillHistogram(dirname,"Phi_v_Theta",140,20,55,janusTheta,200,-200,200,janusPhi);
    }
    else{
      obj.FillHistogram(dirname,"Phi_v_Theta",160,130,170,janusTheta,200,-200,200,janusPhi);
    }
