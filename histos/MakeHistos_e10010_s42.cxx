...
void MakeHistograms(TRuntimeObjects& obj) {
  ...
  if(s800) {
    ...
    if(gretina) {
      ...
      HandleGretina(obj, incoming_s42, outgoing_s42, gt_time);

      HandleAddback(obj, incoming_s42, outgoing_s42, gt_time);

      HandleSegmentAddback(obj, incoming_s42, outgoing_s42, gt_time);
	...
	for(int j=0;j<5;j++) {
	  if((bits>>j)&0x0001) {
	    ...
	    obj.FillHistogram(dirname,histname,
			      ...
			      1000, 0, 8000,
			      hit.GetCoreEnergy());
	    if(incoming_s42 && !outgoing_s42 &&
	       incoming_s42->IsInside(s800->GetOBJ_E1Raw_MESY(),
				      s800->GetXF_E1Raw_MESY()) &&
	       outgoing_s42->IsInside(s800->GetCorrTOF_OBJ_MESY(),
				      s800->GetIonChamber().GetSum()) ){
	      //		      s800->GetIonChamber().Charge()) ){

	      histname = Form("dtime_all_reg%i_%s_%s",j,
			      incoming_s42->GetName(),
			      outgoing_s42->GetName());
	      obj.FillHistogram(dirname,histname,
				500, -250, 250,
				s800->GetTimestamp()-hit.GetTimestamp(),
				1000, 0, 8000,
