...
void BasicJanusSpectra(TRuntimeObjects& obj, TJanusDDASHit& hit, std::string dirname, std::string h_suf) {
  ...
  obj.FillHistogram(dirname,Form("%sRing_det%d_timemap",h_suf.c_str(),hit.GetDetnum()),60,0,1200000000000,hit.Timestamp(),
                    26,0,26,hit.GetRing());
...
void MakeJanus(TRuntimeObjects& obj, TJanusDDAS& janus) {
  ...
  if(prev_ts > 0) {
    obj.FillHistogram(dirname,"Event_TDiff",700000,0,700000000,janus.Timestamp()-prev_ts);
  }
  prev_ts = janus.Timestamp();
  
  obj.FillHistogram(dirname,"Janus_Timestamp",60,0,3600000000000,janus.Timestamp());
  ...
  for(size_t i=0; i<janus.GetChannels().size(); i++) {
    ...
    if(prev_chan_ts > 0) {
      obj.FillHistogram(dirname,"Chan_TDiff",700000,0,700000000,chan.Timestamp()-prev_chan_ts);
    }
    prev_chan_ts = chan.Timestamp();
    if(chan.IsRing()) {
      obj.FillHistogram(dirname,Form("Energy_Time_Ring%d",chan.GetSector()),100,0,2000000000000,chan.Timestamp(),
                        3000,0,30000,chan.Charge());
    }
    else {
      obj.FillHistogram(dirname,Form("Energy_Time_Sector%d",chan.GetSector()),100,0,2000000000000,chan.Timestamp(),
                        3000,0,30000,chan.Charge());
    }
    ...
    for(size_t j=i+1;j<janus.GetChannels().size();j++) {
      ...
      if(chan1.GetDetnum() == chan.GetDetnum()) {
        obj.FillHistogram(dirname,Form("Tdiff_UsableChans_det%d",chan.GetDetnum()),
                          2500,-11000,11000,chan.Timestamp()-chan1.Timestamp());
        obj.FillHistogram(dirname,Form("Tdiff_v_Timestamp_det%d",chan.GetDetnum()),
                          100,0,2000000000000,chan.Timestamp(),
                          625,-11000,11000,chan.Timestamp()-chan1.Timestamp());
      }
    }
  ...
  int ringNum;
  for(auto& ring : rings) {
    ringNum = ring.GetSector();
    if(prev_ring_ts[ringNum-1] > 0) {
      obj.FillHistogram(dirname,Form("Ring%d_TDiff_det%d",ringNum,ring.GetDetnum()),100000,0,10000000000,ring.Timestamp()-prev_ring_ts[ringNum-1]);
    }
    prev_ring_ts[ringNum-1] = ring.Timestamp();
  }
...
void MakeGatedJanus(TRuntimeObjects& obj, TJanusDDAS& janus, GCutG* gate, GCutG* janus_time_gate1, GCutG* janus_time_gate2, GCutG* janus_charge_gate) {
  ...
  for(auto& hit : janus.GetHits(lvl)) {
    ...
    if(hit.GetDetnum() == 1 && hit.GetRing() <= 8 && !janus_time_gate1->IsInside(hit.GetBackHit().Timestamp() - hit.Timestamp(),hit.Charge())) {
      continue;
    }
    if(hit.GetDetnum() == 1 && hit.GetRing() <= 8 && !janus_time_gate1->IsInside(hit.GetBackHit().Timestamp() - hit.Timestamp(),hit.GetBackHit().Charge())) {
      continue;
    }
    if(hit.GetDetnum() == 1 && hit.GetRing() > 8 && !janus_time_gate2->IsInside(hit.GetBackHit().Timestamp() - hit.Timestamp(),hit.Charge())) {
      continue;
    }
    if(hit.GetDetnum() == 1 && hit.GetRing() > 8 && !janus_time_gate2->IsInside(hit.GetBackHit().Timestamp() - hit.Timestamp(),hit.GetBackHit().Charge())) {
      continue;
    }
    if(hit.GetDetnum() == 1 && !janus_charge_gate->IsInside(hit.GetBackHit().Charge(),hit.Charge())) {
      continue;
    }
...
/*
  ...
  for(auto& hit : janus.GetHits(lvl)) {
    ...
    if(hit.GetDetnum() == 1 && !janus_time_gate1->IsInside(hit.GetBackHit().Timestamp() - hit.Timestamp(),hit.Charge())) {
      continue;
    }
    if(hit.GetDetnum() == 1 && !janus_time_gate1->IsInside(hit.GetBackHit().Timestamp() - hit.Timestamp(),hit.GetBackHit().Charge())) {
      continue;
    }

    multRing8++;
    BasicJanusSpectra(obj,hit,dirname,"Ring8_");
  ...
  for(auto& hit : janus.GetHits(lvl)) {
    ...
    if(hit.GetDetnum() == 0 || !janus_time_gate1->IsInside(hit.GetBackHit().Timestamp() - hit.Timestamp(),hit.Charge())) {
      continue;
    }
    if(hit.GetDetnum() == 0 || !janus_time_gate1->IsInside(hit.GetBackHit().Timestamp() - hit.Timestamp(),hit.GetBackHit().Charge())) {
      continue;
    }

    multRing1++;
    BasicJanusSpectra(obj,hit,dirname,"Ring1_");
  ...
  for(auto& hit : janus.GetHits(lvl)) {
    ...
    if(hit.GetDetnum() == 0 || !janus_time_gate2->IsInside(hit.GetBackHit().Timestamp() - hit.Timestamp(),hit.Charge())) {
      continue;
    }
    if(hit.GetDetnum() == 0 || !janus_time_gate2->IsInside(hit.GetBackHit().Timestamp() - hit.Timestamp(),hit.GetBackHit().Charge())) {
      continue;
    }

    multRing12++;
    BasicJanusSpectra(obj,hit,dirname,"Ring12_");
...
void MakeSeGAJanus(TRuntimeObjects& obj, TSega& sega, TJanusDDAS& janus, GCutG* janus_time_gate1, GCutG* janus_time_gate2, GCutG* janus_charge_gate) {
  ...
  for(auto& j_hit : janus.GetHits(lvl)) {
    if(j_hit.GetDetnum() == 1 && j_hit.GetRing() <= 8 && !janus_time_gate1->IsInside(j_hit.GetBackHit().Timestamp() - j_hit.Timestamp(),j_hit.Charge())) {
      continue;
    }
    if(j_hit.GetDetnum() == 1 && j_hit.GetRing() <= 8 && !janus_time_gate1->IsInside(j_hit.GetBackHit().Timestamp() - j_hit.Timestamp(),j_hit.GetBackHit().Charge())) {
      continue;
    }
    if(j_hit.GetDetnum() == 1 && j_hit.GetRing() > 8 && !janus_time_gate2->IsInside(j_hit.GetBackHit().Timestamp() - j_hit.Timestamp(),j_hit.Charge())) {
      continue;
    }
    if(j_hit.GetDetnum() == 1 && j_hit.GetRing() > 8 && !janus_time_gate2->IsInside(j_hit.GetBackHit().Timestamp() - j_hit.Timestamp(),j_hit.GetBackHit().Charge())) {
      continue;
    }
    if(j_hit.GetDetnum() == 1 && !janus_charge_gate->IsInside(j_hit.GetBackHit().Charge(),j_hit.Charge())) {
      continue;
    }
    ...
    if(j_hit.GetRing() == 8) {
      BasicJanusSpectra(obj,j_hit,dirname,"Ring8_");
    }
...
void MakeGatedSeGAJanus(TRuntimeObjects& obj, TSega& sega, TJanusDDAS& janus, GCutG* janus_gate,
			GCutG* time_gate, GCutG* janus_time_gate1, GCutG* janus_time_gate2, GCutG* janus_charge_gate) {
  ...
  for(auto& j_hit : janus.GetHits(lvl)) {
	...
	if(!time_gate->IsInside(tdiff,s_energy)) {
      ...
      obj.FillHistogram(dirname,Form("Doppler_Energy_v_tdiff_jdet%d",detNum),
			1000,-1000,3000,tdiff,
			2500,0,5000,dop_en);
      obj.FillHistogram(dirname,Form("Doppler_Energy_v_janus_tdiff_jdet%d",detNum),
			1000,-1000,3000,j_hit.GetBackHit().Timestamp() - j_hit.Timestamp(),
			2500,0,5000,dop_en);
...
int gates_loaded=0;
GCutG* time_gate=0;
GCutG* janus_time_gate1;
GCutG* janus_time_gate2;
GCutG* janus_charge_gate;
...
void MakeHistograms(TRuntimeObjects& obj) {
  ...
  if(gates_loaded!=gates->GetSize()) {
	...
	gates_loaded++;
      ...
      else if(!tag.compare("janus1")) {
        janus_time_gate1 = gate;
	gates_loaded++;
        std::cout << "Janus Time Gate Ring 1: " << name << std::endl;
      }
      else if(!tag.compare("janus2")) {
        janus_time_gate2 = gate;
	gates_loaded++;
        std::cout << "Janus Time Gate Ring 2: " << name << std::endl;
      }
      else if(!tag.compare("charge")) {
        janus_charge_gate = gate;
        gates_loaded++;
        std::cout << "Janus Charge Gate: " << name << std::endl;
      }
...
*/
  ...
  if(janus) {
    ...
    for(auto &gate : janus_gates) {
      MakeGatedJanus(obj,*janus,gate,janus_time_gate1,janus_time_gate2,janus_charge_gate);
    }
  }

  if(sega && janus) {

    MakeSeGAJanus(obj,*sega,*janus,janus_time_gate1,janus_time_gate2,janus_charge_gate);
    for(auto &gate : janus_gates) {
      MakeGatedSeGAJanus(obj,*sega,*janus,gate,time_gate,janus_time_gate1,janus_time_gate2,janus_charge_gate);
    }
  }
