...
//      obj.FillHistogram(histname,400,-400,400,hit2.Timestamp()-hit.Timestamp());
    ...
    if(gretina) {
        ...
        // gretina->CleanHits();
        //double sumsegener = 0 ;
        static long tstart = -10;
        for(unsigned int i=0; i<gretina->Size(); i++) {
            TGretinaHit hit = gretina->GetGretinaHit(i);
            if(tstart<=0) tstart = hit.Timestamp();
            //hit.Print();
            //hit.TrimSegments(0);
            //hit.Print();
            ...
            long tdiff = (long) (hit.Timestamp() - tstart);
            long mod = 1e8;
            long tdiff1 = tdiff % mod;
            if(quad ==1 || quad == 10)
            obj.FillHistogram(Form("q%02i_p%i_et_nomod",quad,p),10000,0,10000,hit.GetCoreEnergy(),3600,0,3600,tdiff/1e8*60.);
            if(quad ==1 || quad == 10)
            obj.FillHistogram(Form("q%02i_p%i_et",quad,p),10000,0,10000,hit.GetCoreEnergy(),1000,0,1e8,tdiff1);
            mod = long(1e8 / 30. );
            long tdiff2 = tdiff % mod;
            obj.FillHistogram(Form("q%02i_p%i_et2",quad,p),10000,0,10000,hit.GetCoreEnergy(),50,0,1e7,tdiff2);
...
//histname = Form("Gretina_Phi_Id_Cry%02i_Det%02i",hit.GetCrystalId(),hole);
            ...
            obj.FillHistogram(Form("q%02i_p%i_timestamp",quad,p),7200,0,7200,bank29->Timestamp()*1e-8,10000,0,10000,hit.GetCoreEnergy());
