...
void MakeJanus(TRuntimeObjects& obj, TJanusDDAS& janus) {
  ...
  for(auto& chan : janus.GetAllChannels()) {
    ...
    if(chan.GetFrontChannel() == 0) {
      obj.FillHistogram(dirname,"Jan_Time",1000000,0,100000000,chan.Timestamp());
      std::cout << "jan " << chan.Timestamp() << std::endl;
    }
...
/*
    obj.FillHistogram(dirname,Form("CoreEnergy_v_time_det%02d",hit.GetDetnum()),
                      100,0,10000000000000,hit.Timestamp(),
                      1000,0,4000,hit.GetEnergy());
*/

    obj.FillHistogram(dirname,Form("CoreCharge_v_time_det%02d",hit.GetDetnum()),
                      2000,0,200000000000000,hit.Timestamp(),
                      5000,0,33000,hit.Charge());

    if(hit.GetDetnum() == 13) {
      obj.FillHistogram(dirname,"SegaTime",1000000,0,100000000,hit.Timestamp());
      std::cout << "seg " << hit.Timestamp() << std::endl;
    }
...
void MakeSeGAJanus(TRuntimeObjects& obj, TSega& sega, TJanusDDAS& janus) {
  ...
  obj.FillHistogram(dirname,"janMult",40,0,40,janus.GetAllChannels().size());
  ...
  for(auto& chan : janus.GetAllChannels()) {
    if(chan.GetFrontChannel() == 0) {
      obj.FillHistogram(dirname,"janus_ts",10000,0,10000000000,chan.Timestamp());
    }
  }

  for(auto& s_hit : sega) {
    if(s_hit.GetDetnum() == 13) {
      obj.FillHistogram(dirname,"sega13_ts",10000,0,10000000000,s_hit.Timestamp());
      for(auto& chan : janus.GetAllChannels()) {
        auto tdiff = (s_hit.Timestamp() - chan.Timestamp());
        obj.FillHistogram(dirname,"Timing_Difference_Test",5000,-10000,10000,tdiff);
      }
    }
  } 
