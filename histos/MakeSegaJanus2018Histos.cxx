...
void MakeGatedCoincidenceHistograms(TRuntimeObjects& obj, TSega& sega, TJanusDDAS& janus, GCutG* tcut, GCutG * scatteredRb, GCutG *sectorCut);
...
void MakeHistograms(TRuntimeObjects& obj) {
    ...
    if(gates_loaded!=gates->GetSize()) {
        ...
        while(TObject *obj = iter.Next()) {
            ...
            if(!tag.compare("scattered")) {//gate on kinematic line in Ring Energy v. Angle
                scatteredRb = new GCutG(*gate);
                std::cout << "scattered: << " << gate->GetName() << std::endl;
            } else if(!tag.compare("time")){//Gamma Energy Time cut
                tcut = new GCutG(*gate);
                std::cout << "time: << " << gate->GetName() << std::endl;
            }  else if(!tag.compare("sector")){//Gate on kinematic line in Sector energy v. angle
                sectorGates.push_back(new GCutG(*gate));
                std::cout << "sector gate: << " << gate->GetName() << std::endl;
            } else {
...
void MakeJanusHistograms(TRuntimeObjects& obj, TJanusDDAS& janus) {
    ...
    for(auto& hit : janus.GetAllHits()) {
        ...
        obj.FillHistogram("janus", Form("det%d_frontback_tdiff_sectorenergy", hit.GetDetnum()),
                2000, -1000, 1000, tdiff,
...
void MakeGatedCoincidenceHistograms(TRuntimeObjects& obj, TSega& sega, TJanusDDAS& janus, GCutG *tcut, GCutG* scattered, GCutG *sectorCut) {

    std::string dirname = "coinc_tcut_scatteredRb";
    if (sectorCut != nullptr){
            dirname += "_";
            dirname += sectorCut->GetName();

    }

    for(auto& j_hit : janus.GetAllHits()) {
        if (sectorCut != nullptr){
            if (!sectorCut->IsInside(j_hit.GetPosition().Theta()*(180./TMath::Pi()),
                                    j_hit.GetBackHit().GetEnergy())){
                continue;
            }
        }
        for(unsigned int i=0; i<sega.Size(); i++) {
