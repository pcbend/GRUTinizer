...
void MakeJanus(TRuntimeObjects& obj, TJanusDDAS& janus, GCutG* charge_gate) {
  ...
  for(auto& chan : janus.GetAllChannels()) {
    ...
    obj.FillHistogram(dirname,"Charge_Summary_AllChans",
                      128,0,128,ringNum,
                      3500,0,35000,chan.Charge());
    obj.FillHistogram(dirname,"Charge_Summary_AllChans_test",
                      128,0,128,chan.GetFrontChannel(),
                      3500,0,35000,chan.Charge());
...
*/
  ...
  if(janus) {
    MakeJanus(obj,*janus,charge_gate);
