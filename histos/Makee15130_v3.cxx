...
void SetGates(TRuntimeObjects &obj) {
  ...
  while(TObject *key = iter.Next()) {
    ...
    if(strcmp(gate->GetName(),"gg_time")==0) {
      gg_time = gate;
    }
  }
  gates_set=true;
           ...
           while(TObject *key = iter.Next()) {
             ...
             if(gate->IsInside(xvalue,yvalue)) {
                if(fill[l] == false){
                  ...
                  obj.FillHistogram(dirname,Form("gamma_time_uml_m2_%s",gate->GetName()),1000,-twin/100.,twin/100.,(tmp_hit.Timestamp()-fts)/100,2000,0,4000,tmp_hit.GetCoreEnergy());
                  fill[l] = true;
                }
                if(fill[l+1] == false){
        ...
        if(store_ghit.at(l+2)->first-store_ghit.at(l)->first < 100){
           while(TObject *key = iter.Next()) {
             ...
             if(gate->IsInside(xvalue,yvalue)) {
                if(fill[l] == false){
                  TGretinaHit tmp_hit = store_ghit.at(l)->second;
                  obj.FillHistogram(dirname,Form("gamma_time_uml_m3_%s",gate->GetName()),3000,-twin/100,twin/100,(tmp_hit.Timestamp()-fts)/100,2000,0,4000,tmp_hit.GetCoreEnergy());
                  fill[l] = true;
                }
                if(fill[l+1] == false){
                  TGretinaHit tmp_hit = store_ghit.at(l+1)->second;
