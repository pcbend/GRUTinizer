...
void getScalerCounts(const char *input_root_file_name, const int final_entry){
  ...
  std::vector<int> scaler_32(32,0);//periodic scaler
  ...
  for (int entry = 0; entry < n_entries; entry++){
    ...
    if (scalers->Size() == 32){
      for (int i = 31; i >= 0;i--){
	    ...
	    if(scalers->GetScaler(14) > prev_10hz) {
              if (scalers->GetScaler(i) < scaler_32.at(i)){
                scaler_32_overflows.at(i) += 1;
              }
            scaler_32.at(i) = scalers->GetScaler(i);
            } //Prevent overflow due to out of order packets, 10HZ clock should never overflow
