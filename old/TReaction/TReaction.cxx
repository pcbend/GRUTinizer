...
TGraph *TReaction::RutherfordVsTheta(double thmin, double thmax, int part, bool Frame_Lab, bool Units_mb){
	...
	g->SetTitle(Form("Rutherford cross section for %s; Theta_{%s} [deg]; dSigma / dOmega_{%s} [%s]",
                   GetName(),frame,frame,Units_mb?"mb":"fm^2"));
...
// Frame_Lab -> dSigma/dOmegaLab[ThetaLab] 	and 	Frame_Cm -> dSigma/dOmegaCm[ThetaCm]
TGraph *TReaction::RutherfordVsTheta_SinTheta(double thmin, double thmax, int part, bool Frame_Lab, bool Units_mb){
  TGraph* g = RutherfordVsTheta(thmin, thmax, part, Frame_Lab, Units_mb);

  const char *frame = Form("%s",Frame_Lab?"Lab":"Cm");
  g->SetTitle(Form("Rutherford cross section for %s; Theta_{%s} [deg]; dSigma / dTheta_{%s} [%s]",
                   GetName(),frame,frame,Units_mb?"mb":"fm^2"));

  for(int i=0; i<g->GetN(); i++){
    double theta_deg = g->GetX()[i];
    double value = g->GetY()[i];
    value *= 2*TMath::Pi()*std::sin((TMath::Pi()/180.0) * theta_deg);
    g->SetPoint(i, theta_deg, value);
  }
  return g;
}
