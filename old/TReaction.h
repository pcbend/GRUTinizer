...
public:
  ...
  // This DOES NOT include the 2*pi*sin(theta) term.
  TGraph *RutherfordVsTheta(double thmin = 1.0, double thmax = 179.0, int part = 2, bool Frame_Lab = true, bool Units_mb = true);
  // Frame_Lab -> dSigma/dThetaLab[ThetaLab] 	and 	Frame_Cm -> dSigma/dThetaCm[ThetaCm].
  // This DOES include the 2*pi*sin(theta) term.
  TGraph *RutherfordVsTheta_SinTheta(double thmin = 1.0, double thmax = 179.0, int part = 2, bool Frame_Lab = true, bool Units_mb = true);
