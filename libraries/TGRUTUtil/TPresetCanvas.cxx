...
void TPresetPad::Draw(Option_t *opt) {
	...
	if(!obj) {
		...
		TString sname = this->GetObjName();
		if(sname.Contains("/")) {
			printf("sname = %s\n",sname.Data());
			TObjArray *objarr = sname.Tokenize('/');
			if(objarr->GetEntries()) {
				std::string dname= ((TObjString*)objarr->First())->GetName();
				std::string oname= ((TObjString*)objarr->Last())->GetName();

				if(TRuntimeObjects::Get()) {
					TList *list = ((TList*)TRuntimeObjects::Get()->GetObjects().FindObject(dname.c_str()));
					obj = list->FindObject(oname.c_str());
